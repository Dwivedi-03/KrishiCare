...
function validateContactForm() {
  ...
  if (result) {
    ...
    var dataForm =
      "email=" + email + "&message=" + message + "&process=registerContact";
...
function verifyUser(labId) {
  var result =
    validateDropdown("requestid", "Request Id", "spanrequestid") &&
...
function ApproveLab(labid, email) {
  ajaxCall(
    "../Backend/Register.php",
    "post",
    "labid=" + labid + "&email=" + email + "&process=approveLab",
    "approveLab",
    true
  );
...
function validateDropdown(elementId, elementName, spanid) {
  ...
  if (element.value == "default") {
    ...
    document.getElementById(spanid).textContent =
      elementName + " should be selected!";
    return false;
  } else {
    document.getElementById(spanid).textContent = "";
    return true;
...
function CheckPassword(elementId, elementName, spanid) {
  ...
  if (res) {
    ...
    if (input.value.match(passw)) {
      ...
      document.getElementById(elementId).focus();
      document.getElementById(spanid).textContent =
        elementName + " should be of 7 to 14 character!";
      return false;
    }
  }
}
...
function acceptRequest(farmerId, labId) {
  ajaxCall(
    "../Backend/LabProcess.php",
    "post",
    "farmerId=" + farmerId + "&labId=" + labId + "&process=changeStatus",
    "requestStatus",
    true
  );
...
function initializeCarousel() {
  ...
  function moveToPrev() {
    currentIndex =
      (currentIndex - 1 + carouselItems.children.length) %
      carouselItems.children.length;
